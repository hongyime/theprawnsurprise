...
2026-09-15: PR #180 reconciles the shared heartbeat and shallow LFS guard while preserving the current Python action version and pinned labeler SHA. The heartbeat validates its configured root and refuses collisions or tampered branch contents. LFS scan errors fail the check. Required Build and Vercel checks gate release; application code, dependencies and asset caching are unchanged.
